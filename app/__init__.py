...
def register():
        ...
        if request.method == "POST":
            ...
            if username and password:
                return authorize.register(username, password)
            else:
...
@authorize.user_is_login
@authorize.bucketlist_exist
@authorize.belongs_to_user
def manage_single_bucketlist(id):
    if request.method == 'GET':
        response = {"data": authorize.get_single_bucketlist(id)}
        print(response)
        return jsonify(response)
    elif request.method == 'PUT':
        name = request.form.get("name")
        if name:
            return authorize.update_bucketlist(id,name)
        else:
            return jsonify({"message": "Please supply bucketlist name"})
    elif request.method == 'DELETE':
        return authorize.delete_bucket_list(id)
    else:
...
def manage_bucketlist():
    ...
    user_id = authorize.get_current_user_id(token)
    name = request.form.get("name")
    if token:
        if request.method == 'GET':
            response = authorize.get_all_bucketlist(user_id)
            
            return jsonify(response)
        elif request.method == 'POST':
            if name:
                return authorize.create_bucketlist(name=name,created_by=user_id)
            else:
                return jsonify({"message": "Please supply bucketlist name"})
        else:
...
def create_bucketlist_item(id):
    ...
    if name:
        return authorize.create_bucketlist_item(id,name,done=False)
    else:
